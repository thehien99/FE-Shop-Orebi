...
const SideBar = ({ closeSidebar }) => {
  ...
  return (
    <div className="sidebar_admin p-6 xs:p-2">
            ...
            {menuSidebar.map((item) => {
              return (
                <NavLink
                  ...
                  key={item.id}
                  state={item.name}
                  className={({ isActive }) => isActive ? 'text-yellow-500 flex items-center gap-3 xs:w-full' : 'flex items-center gap-3 xs:w-full '}
                  onClick={closeSidebar}  // Đóng sidebar khi chọn mục
                >
                  <div> {item.icon}</div>
                  <div className="font-sans lg:text-[17px]">
                    {item.name}
                  </div>
                </NavLink>
              )
